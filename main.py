...
conn = connect(
    host="localhost", database="fastapi_db", user="postgres", password="12345"
)
...
async def register_user(request: Request):
    ...
    cursor.execute(
        "INSERT INTO users (full_name, password, email, phone) VALUES (%s, %s, %s, %s) RETURNING id",
...
async def get_user_details(user_id: str):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT full_name, email, phone FROM users WHERE id = %s", (user_id,)
    )
